...
void maybeReloadOIDCFileConfig()
{
   ...
   {
      std::lock_guard<std::mutex> lock(ConfigMtx);
      if (OIDCConfigStatValid && sfr.ino == OIDCConfigIno && sfr.mtime == OIDCConfigMTime) return;
   }
   ...
   for (const auto &kobj : *kIt)
      {
       ...
       if (!pkey) continue;
       auto old = keys.find(kid);
       if (old != keys.end()) EVP_PKEY_free(old->second);
       keys[kid] = pkey;
      }
   if (keys.empty())
            ...
            {     if (!strcmp(val, "-maxsz"))
                     {if (!(val = cfg.GetToken()))
                      ...
                      {int msz = strtol(val, &endP, 10);
                       if (*endP == 'k' || *endP == 'K') {msz *= 1024; endP++;}
                       MaxTokSize = msz;}
