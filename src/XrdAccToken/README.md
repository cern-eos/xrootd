...
JWT scope claims on storage paths. It works together with `XrdSecOIDC`, which
handles **authentication** and JWT validation.

The plugin does not parse or verify tokens itself. After `XrdSecOIDC` has
...
```
Client ──► XrdSecOIDC (authenticate, validate JWT)
              │
              ▼
         XrdSecEntity
         • name        (identity)
         ...
         • base_path   (optional issuer namespace, from OIDC)
         • restricted_path (optional JSON path allow-list, from OIDC)
              │
              ▼
...
- **Site base path** — optional `acctoken.basepath` prefix applied to all scope
  paths; when the entity also carries OIDC `base_path`, the configured base
  path is applied first and the entity value is appended
  (e.g. site `/atlas` + entity `/eos` + scope `/data` → `/atlas/eos/data`)
- **Restricted paths** — when OIDC exports `restricted_path` as a JSON array,
...
- **HTTP support** — works with `root://` and HTTPS when OIDC is configured
...
## XrdSecOIDC configuration

`XrdAccToken` requires `XrdSecOIDC` (`sec.protocol oidc`) to authenticate
...
# OIDC authentication — export claims needed by XrdAccToken
sec.protparm oidc -issuer https://iam.example.org
sec.protparm oidc -audience https://storage.example.org
...
`XrdSecoidc` normalizes arrays to a space-separated `token.scope` value.
...
See `src/XrdSecoidc/README.md` and `src/XrdSecoidc/configs/wlcg-iam.example.cfg`
for a full IAM example with `XrdAccToken`.

### Optional: issuer `base_path` and `restricted_path`

When configured per issuer in `sec.protocol oidc` or `oidc.cfg`, OIDC exports
...
[global]
expiry = required
entity-claim = scope, wlcg.ver

[issuer "https://iam.example.org"]
audience = https://storage.example.org
```

With inline `sec.protparm oidc` lines, list all parameters **before**
`sec.protocol oidc`. See the [XrdSecOIDC README](../XrdSecoidc/README.md) for
full OIDC options (issuers, audiences, JWKS, identity mapping, HTTP setup).

### HTTPS clients

For HTTPS access with the same OIDC configuration:
...
When the entity has no scope attribute (e.g. a non-OIDC session, or OIDC
...
sec.protparm oidc -issuer https://iam.example.org
sec.protparm oidc -audience eos-storage
sec.protparm oidc -expiry required
sec.protparm oidc -entity-claim scope
sec.protparm oidc -entity-claim wlcg.ver
sec.protocol oidc
...
| - | ----------- | ------------ |
| JWT validation | via XrdSecOIDC | built-in (libscitokens) |
| Configuration | `sec.protocol oidc` + `ofs.authlib` | `ofs.authlib` + `scitokens.cfg` |
...
| Issuer/JWKS config | in OIDC plugin | in scitokens.cfg |

`XrdAccToken` is suited for deployments that already use `XrdSecOIDC` and want
