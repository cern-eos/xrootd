...
void releaseGssName(gss_name_t &name)
{
...
bool importServicePrincipal(const std::string &kprinc, gss_name_t *gssName,
                            std::string &errMsg)
{
  ...
  const std::string namePart =
      (at == std::string::npos) ? kprinc : kprinc.substr(0, at);
  auto slash = namePart.find('/');

  if (slash != std::string::npos) {
    const std::string hostbased =
        namePart.substr(0, slash) + "@" + namePart.substr(slash + 1);
    nameBuf.length = hostbased.size();
    nameBuf.value  = (void *)hostbased.c_str();
    maj = gss_import_name(&min, &nameBuf, GSS_C_NT_HOSTBASED_SERVICE, gssName);
  } else {
                       ...
                       const char *principal)
{
  ...
  if (!importServicePrincipal(kprinc, &gssName, importErr)) {
    eDest.Emsg(TraceID, "Unable to import Kerberos principal:",
               kprinc.c_str(), importErr.c_str());
    return false;
  }

  gss_cred_id_t acquired = GSS_C_NO_CREDENTIAL;
  OM_uint32 maj, min;
  maj = gss_acquire_cred(&min, gssName, GSS_C_INDEFINITE, GSS_C_NO_OID_SET,
                         GSS_C_ACCEPT, &acquired, nullptr, nullptr);
  releaseGssName(gssName);
                                            ...
                                            std::string &errMsg)
{
  ...
  maj = gss_accept_sec_context(&min, (gss_ctx_id_t *)&ctx_,
