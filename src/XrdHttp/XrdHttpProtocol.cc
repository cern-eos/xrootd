...
int XrdHttpProtocol::xauth(XrdOucStream &Config) {
  ...
  if(val) {
    if(!strcmp("tpc",val)) {
      ...
      if (!keytab || !*keytab) {
        eDest.Emsg("Config", "http.auth krb5 requires a keytab.");
        return 1;
      }
      std::string kt = keytab;
      char *principal = Config.GetWord();
      if (!principal || !*principal) {
        eDest.Emsg("Config", "http.auth krb5 requires a principal.");
        return 1;
      }
      std::string pc = principal;
      if (!XrdHttpKrb5::Init(eDest, kt.c_str(), pc.c_str()))
        return 1;
#endif
    } else {
